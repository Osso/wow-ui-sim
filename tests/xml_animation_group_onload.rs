...
fn forever_dock_overflow_highlight_keeps_its_flash_animation() {
    ...
    env.exec(
        ...
        assert(highlight.FlashAnim:GetParent() == highlight, "animation owner mismatch")
        assert(select("#", highlight:GetAnimationGroups()) == 1, "duplicate groups: " .. select("#", highlight:GetAnimationGroups()))
        highlight.FlashAnim:Play()
        assert(highlight.FlashAnim:IsPlaying())
        ChatFrameUtil.StopFlash(highlight, highlight.FlashAnim, true)
        assert(highlight:IsShown(), "StopFlash did not show highlight")
        highlight.FlashAnim:Play()
        assert(FCFDockOverflowButton_UpdatePulseState(button), "consumer did not update")
        assert(not highlight.FlashAnim:IsPlaying(), "consumer did not stop animation")
        assert(highlight:GetAlpha() == 1, "consumer did not reset alpha")
