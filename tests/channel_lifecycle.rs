...
fn channel_inputs_validate_atomically_and_isolate_modes() {
    ...
    env.exec(r#"
        ...
        local empowerID = select(11, UnitChannelInfo('player'))
        assert(A_Admin.UpdateEmpower({0.5, 0.5, 0.5}, 0.5))
        assert(select(10, UnitChannelInfo('player')) == 3)
        assert(select(11, UnitChannelInfo('player')) == empowerID)
        assert(GetUnitEmpowerStageDuration('player', 2) == 500)
        assert(GetUnitEmpowerStageDuration('player', 3) == nil)
        assert(GetUnitEmpowerHoldAtMaxTime('player') == 500)
        assert(not pcall(A_Admin.UpdateEmpower, {1}, -1))
        assert(select(10, UnitChannelInfo('player')) == 3)
