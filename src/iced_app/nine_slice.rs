...
fn emit_horiz_edges(batch: &mut QuadBatch, bounds: Rectangle, ns: &NineSliceAtlasInfo, alpha: f32) {
    ...
    emit_horiz_tiles(batch, top_bounds, &piece_uvs(&ns.edge_top), ns.edge_top.file, ns.edge_top.width as f32, alpha);
    ...
    emit_horiz_tiles(batch, bot_bounds, &piece_uvs(&ns.edge_bottom), ns.edge_bottom.file, ns.edge_bottom.width as f32, alpha);
}
...
fn emit_vert_edges(batch: &mut QuadBatch, bounds: Rectangle, ns: &NineSliceAtlasInfo, alpha: f32) {
    ...
    emit_vert_tiles(batch, left_bounds, &piece_uvs(&ns.edge_left), ns.edge_left.file, ns.edge_left.height as f32, alpha);
    ...
    emit_vert_tiles(batch, right_bounds, &piece_uvs(&ns.edge_right), ns.edge_right.file, ns.edge_right.height as f32, alpha);
}
