    ...
    let create_frame = lua.create_function(move |lua, args: mlua::MultiValue| {
        ...
        if let Some(ref tmpl) = template {
            ...
            if tmpl.contains("PortraitFrameTemplate") || tmpl.contains("PortraitFrameFlatTemplate") {
                ...
                if let Some(tc) = state.widgets.get_mut(title_container_id) {
                ...
                if let Some(parent_frame) = state.widgets.get_mut(frame_id) {
                    parent_frame.children_keys.insert("TitleContainer".to_string(), title_container_id);
                    parent_frame.children_keys.insert("NineSlice".to_string(), nine_slice_id);
                    parent_frame.children_keys.insert("CloseButton".to_string(), close_button_id);
                    parent_frame.children_keys.insert("PortraitContainer".to_string(), portrait_container_id);
                }
            }
