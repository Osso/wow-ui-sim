...
/// Action slot query stubs: text, count, cooldown, state checks.
fn register_action_slot_queries(lua: &Lua) -> Result<()> {
    let globals = lua.globals();
    ...
    globals.set(
        "GetActionCooldown",
        lua.create_function(|_, _slot: Value| Ok((0.0_f64, 0.0_f64, 1, 1.0_f64)))?,
    )?;
