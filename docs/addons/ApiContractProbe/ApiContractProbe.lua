...
local function captureNeighborhoodStructures(mode)
    ...
    if mode == "recraft-limit-read" or mode == "recraft-reagent-read" then
        ...
        local function field(path, object, key)
            if authorize(path) or not accessible(key) then return nil, false end
            return readField(object, key)
        end
        local function objectStatus(value)
            if not accessible(value) then return "restricted-input" end
            if type(value) ~= "table" and type(value) ~= "userdata" then return "unavailable-input" end
        end
        local function reagentStatus(path, reagent)
            local status = authorize(path) or objectStatus(reagent)
            if status then return status end
            local item, itemOK = field(path, reagent, "itemID")
            if not itemOK then return "field-error" end
            if not accessible(item) then return "restricted-input" end
            if item ~= nil then
                status = customSetInputStatus(item, "number")
                if status then return status end
            end
            local currency, currencyOK = field(path, reagent, "currencyID")
            if not currencyOK then return "field-error" end
            if not accessible(currency) then return "restricted-input" end
            if currency ~= nil then
                status = customSetInputStatus(currency, "number")
                if status then return status end
            end
            -- Bounded peer reauthorization, not atomic access against arbitrary guard side effects.
            for _ = 1, 2 do
                status = authorize(path)
        ...
        local function queryReagent(path, reagent)
            ...
            if status then row.result = { status = status }; return row end
            row.fields = { itemID = scalar(item), currencyID = scalar(currency) }
            local fn, ok = readField(C_TradeSkillUI, "RecraftLimitCategoryValid")
            if not ok then row.result = { status = "field-error" }; return row end
            if not accessible(fn) or type(fn) ~= "function" then
                row.result = { status = "missing-api" }; return row
            end
            -- Serialization, API lookup and function guards may revoke any original ancestor or field.
            status = reagentStatus(path, reagent)
            if status then row.result = { status = status }; return row end
            local values = pack(pcall(fn, reagent))
            if not values[1] then row.result = { status = "call-error" }; return row end
            row.result = mapTuple(unpack(values, 2, values.n))
            row.result.status = "observed"
            return row
        end
        local function slotsFor(ids, id, schematic)
            local rows, path = {}, pack(ids, id, schematic)
            if objectStatus(schematic) then return rows end
            local slots, ok = field(path, schematic, "reagentSlotSchematics")
            if not ok or objectStatus(slots) then return rows end
            for s = 1, 2 do
                local slot, slotOK = field(pack(ids, id, schematic, slots), slots, s)
                local row = { slot = slotOK and scalar(slot) or { status = "field-error" }, reagents = {} }
                rows[s] = row
                if slotOK and not objectStatus(slot) then
                    local ancestry = pack(ids, id, schematic, slots, slot)
                    local reagents, reagentsOK = field(ancestry, slot, "reagents")
                    if reagentsOK and not objectStatus(reagents) then
                        for i = 1, 2 do
                            local source = pack(ids, id, schematic, slots, slot, reagents)
                            local reagent, reagentOK = field(source, reagents, i)
                            row.reagents[i] = reagentOK and queryReagent(
                                pack(ids, id, schematic, slots, slot, reagents, reagent), reagent)
                                or { result = { status = "field-error" } }
                        end
                    end
                end
            end
        ...
        local function observeRecipe(ids, id, query)
            local function inputStatus()
                if not accessible(false) then return "restricted-input" end
                if query then
                    local status = authorize(pack(ids, id, nil)) or customSetInputStatus(id, "number")
                    if status then return status end
                    if authorize(pack(false, nil, ids, id, false, nil, ids, id)) then return "restricted-input" end
                end
            end
            local status = inputStatus()
            if status then return { status = status } end
            local fn, ok = readField(C_TradeSkillUI, query and "GetRecipeSchematic" or "GetRecipesTracked")
            if not ok then return { status = "field-error" } end
            if not accessible(fn) or type(fn) ~= "function" then return { status = "missing-api" } end
            status = inputStatus()
            if status then return { status = status } end
            local values
            if query then values = pack(pcall(fn, id, false, nil))
        ...
        for index = 1, 2 do
            local id, ok = field(pack(ids), ids, index)
            local row = { id = ok and scalar(id) or { status = "field-error" }, slots = {} }
            result.entries[index] = row
            if ok then
                local schematicValues
                row.schematic, schematicValues = observeRecipe(ids, id, true)
                if schematicValues then row.slots = slotsFor(ids, id, schematicValues[2]) end
            else row.schematic = { status = "field-error" } end
        end
        return result
    end
    if mode == "crafting-enchant-items" then
        local function inputStatus(id)
            local status = customSetInputStatus(id, "number")
            if status then return status end
            if not accessible(nil) or not accessible(id) or not accessible(nil) then
                return "restricted-input"
            end
            -- The nil guard can revoke the previously checked original recipe ID.
            if not accessible(id) then return "restricted-input" end
        end
        local function inspectItems(list)
            local observation = scalar(list)
            if observation.status ~= "observed" or observation.kind ~= "table" then return observation end
            observation.entries = {}
            for index = 1, 8 do
                local value, ok = readField(list, index)
                observation.entries[index] = ok and scalar(value) or { status = "field-error" }
            end
            return observation
        end
        local function observeRecipe(id, query)
            local status = query and inputStatus(id)
            if status then return { status = status } end
            local fn, ok = readField(C_TradeSkillUI, query and "GetEnchantItems" or "GetRecipesTracked")
            if not ok then return { status = "field-error" } end
            if not accessible(fn) or type(fn) ~= "function" then return { status = "missing-api" } end
            local values
            if query then
                ...
                if status then return { status = status } end
                values = pack(pcall(fn, id, nil))
            else
                if not accessible(false) then return { status = "restricted-input" } end
                values = pack(pcall(fn, false))
            end
            if not values[1] then return { status = "call-error" } end
            local observation = mapTuple(unpack(values, 2, values.n))
            observation.status = "observed"
            if query and values.n > 1 then observation.values[1] = inspectItems(values[2]) end
            return observation, values
        end
        ...
        if first and first.status == "observed" and first.kind == "table" then
            for index = 1, 4 do
                ...
                result.entries[index] = {
                    ...
                    items = ok and observeRecipe(id, true) or { status = "field-error" },
                }
            end
        end
        return result
    ...
    if mode == "timeline-track-queries" then
        local function observeTrackQuery(name, id)
            local query = name == "GetEventTrack"
            if query then
                local input = scalar(id)
                if input.status == "restricted" then return { status = "restricted-input" } end
                if input.status ~= "observed" or input.kind ~= "number" then
        ...
        local producer, values = observeTrackQuery("GetEventList")
        result = { producer = producer, entries = {} }
        local first = producer.values and producer.values[1]
        if first and first.status == "observed" and first.kind == "table" then
            for index = 1, 8 do
                ...
                result.entries[index] = {
                    id = ok and scalar(id) or { status = "field-error" },
                    track = ok and observeTrackQuery("GetEventTrack", id) or { status = "field-error" },
                }
            end
        end
        -- Both reads remain independent of event-list and per-event failures.
        result.trackList, values = observeTrackQuery("GetTrackList")
        first = result.trackList.values and result.trackList.values[1]
        if first and first.status == "observed" and first.kind == "table" then
            result.trackList.entries = {}
            for index = 1, 5 do
                local entry, ok = readField(values[2], index)
                result.trackList.entries[index] = ok and inspectNeighborhoodFields(entry, {
                    "id", "type", "minimumDuration", "maximumDuration", "minimumEventIntroDuration",
                    "minimumEventGapDuration", "maximumEventCount", "sortDirection",
                }) or { status = "field-error" }
            end
        end
        result.visible = observeTrackQuery("HasVisibleEvents")
        return result
    end
...
local function captureTradeskillItemQuality(mode)
    if mode == "recipe-quality-acceptance" then
        local function authorize(values)
            for index = 1, values.n do
                if not accessible(values[index]) then return "restricted-input" end
            end
            -- Bounded peer rechecks, not atomic authorization against arbitrary guard side effects.
            for index = values.n, 1, -1 do
                if not accessible(values[index]) then return "restricted-input" end
            end
        end
        local function observeRecipe(name, ancestors, ...)
            local args = pack(...)
            local function inputStatus()
                local status = authorize(ancestors) or authorize(args)
                if status then return status end
                if name ~= "GetRecipesTracked" then
                    status = customSetInputStatus(args[1], "number")
                    if status then return status end
                end
            ...
            local observation = mapTuple(unpack(values, 2, values.n))
            observation.status = "observed"
            if name == "GetRecipeItemQualityInfo" then observation.info = inspectCraftingQuality(values[2]) end
            return observation, values
        end
        local function captureRecipe(ids, id)
            local row = { id = scalar(id) }
            local values
            row.schematic, values = observeRecipe("GetRecipeSchematic", pack(ids, id), id, false, nil)
            row.query = { status = "unavailable-input" }
            if not values then return row end
            local object = values[2]
            local status = authorize(pack(ids, id, object))
            if status then row.query = { status = status }; return row end
            if type(object) ~= "table" and type(object) ~= "userdata" then return row end
            local quality, ok = readField(object, "productQuality")
            row.productQuality = ok and scalar(quality) or { status = "field-error" }
            if not ok then row.query = { status = "field-error" }; return row end
            -- Forward the one original field value with its original recipe, never a reconstructed tier.
            row.query = observeRecipe("GetRecipeItemQualityInfo", pack(ids, id, object), id, quality)
            return row
        end
        local producer, values = observeRecipe("GetRecipesTracked", pack(), false)
        local result = { producer = producer, entries = {} }
        local ids = values and values[2]
        if not accessible(ids) or type(ids) ~= "table" then return result end
        for index = 1, 4 do
            local id, ok = readField(ids, index)
            result.entries[index] = ok and captureRecipe(ids, id)
                or { id = { status = "field-error" }, schematic = { status = "field-error" },
                    query = { status = "unavailable-input" } }
        end
        return result
    end
    local result = { slots = {} }
...
local function captureAbbreviations(mode)
    if mode == "options" then
        local function produceBreakpoints()
            local fn, ok = readField(C_StringUtil, "GetDefaultAbbreviationBreakpoints")
            if not ok then return { status = "field-error" } end
            if not accessible(fn) or type(fn) ~= "function" then return { status = "missing-api" } end
            if not accessible(nil) then return { status = "restricted-input" } end
            local values = pack(pcall(fn, nil))
            if not values[1] then return { status = "call-error" } end
            local result = mapTuple(unpack(values, 2, values.n))
            result.status = "observed"
        ...
        local producer, breakpoints = produceBreakpoints()
        local options
        if accessible(breakpoints) and type(breakpoints) == "table" then
            options = { breakpointData = breakpoints }
        end
        local function inputStatus(input, withOptions)
            if withOptions then
                if options == nil then return "unavailable-input" end
                if not accessible(options) or not accessible(breakpoints) then return "restricted-input" end
                -- This is our own field, not an entry in the opaque producer table.
                if not accessible(options) then return "restricted-input" end
                local value = rawget(options, "breakpointData")
                if not accessible(value) or not accessible(breakpoints) then return "restricted-input" end
                if not rawequal(value, breakpoints) then return "unavailable-input" end
                if not accessible(options) or not accessible(breakpoints) then return "restricted-input" end
            end
        ...
        local function query(name, input, withOptions)
            ...
            if not ok then return { status = "field-error" } end
            if not accessible(fn) or type(fn) ~= "function" then return { status = "missing-api" } end
            -- Lookup and function guards can revoke any original argument.
            status = inputStatus(input, withOptions)
            if status then return { status = status } end
            local values
            if withOptions then values = pack(pcall(fn, input, options))
            else values = pack(pcall(fn, input)) end
            if not values[1] then return { status = "call-error" } end
            local result = mapTuple(unpack(values, 2, values.n))
            result.status = "observed"
        ...
        local result = { producer = producer, samples = {} }
        for _, input in ipairs({ 0, 999, 1000, 1234, 999999, 1000000, -1000, 1000000000 }) do
            local row = { input = scalar(input), small = {}, large = {} }
            row.small.omitted = query("AbbreviateNumbers", input, false)
            row.small.options = query("AbbreviateNumbers", input, true)
            row.large.omitted = query("AbbreviateLargeNumbers", input, false)
            row.large.options = query("AbbreviateLargeNumbers", input, true)
...
SlashCmdList.APICONTRACTPROBE = function(input)
    ...
    if type(issecretvalue) ~= "function" or type(canaccessvalue) ~= "function" then
        ...
        if mode == "recipe-quality-acceptance" then record.recipeQualityAcceptance = captureTradeskillItemQuality(mode) end
