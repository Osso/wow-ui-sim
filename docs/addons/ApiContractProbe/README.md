...
Manual native observations for scalar curve points, `UnitSexBase`, unit names/realms, publication/CVars and event payloads. Complements [AuraDispelCurveProbe](../AuraDispelCurveProbe/README.md). Neither recorder infers native contracts from fixture outputs.
...
2. Run `/apicontract all normal` to capture curves, sex, names and publication. Use `/apicontract curves`, `/apicontract sex <label>` or `/apicontract names <label>` to capture separately.
