...
- Treat this as input acceptance, never valid-quality, index, tier, maximum, or equivalence evidence. Do not invent a range, floor/clamp/coerce values, reconstruct a recipe, substitute the schematic's `recipeID`, or compare against returned quality. Nullable, inaccessible, malformed or failed producers suppress only dependent calls; raw query errors/nil results remain observations and peers continue.
- Guard original list/ID/schematic/value before inspection and forwarding, including after namespace/function lookup and function guards. Reauthorize original quality and list/schematic ancestors after the final argument pass, with bounded original-argument peer rechecks; no atomic-authorization claim under arbitrary guard side effects. Preserve the originally read `productQuality`, without rereading or replacing it after observations. Recheck each result object before every field access, then guard each field before serialization.
- Save raw `recipeQualityAcceptance.producer` and positional `entries[].id/schematic/productQuality/query` observations. Inspect only the first quality result and exactly thirteen declared fields: `quality`, `icon`, `iconSmall`, `iconInventory`, `iconMixed`, `iconAppear`, `iconDissolve`, `barFill`, `barBackground`, `barBackgroundCap`, `barHighlight`, `iconChat`, `iconQuestObjective`. Preserve exact arity/nil positions, opaque errors and inaccessible results; no raw object retention or traversal beyond these fixed paths.
- Cap nine experiment API calls, ten shared snapshots, sixteen tuple positions, 256-byte scalar strings and 128-byte labels. Exclude crafting/recrafting, requests, allocation, setters, reagent-link queries, mutations and native/atlas/error-semantic conclusions. Eighteen local actual TOC/slash fixtures, including 24 frozen ninth-ID-check and 288 staged cross-input cases, cover recorder mechanics only, not native behavior or final acceptance. Existing schematic/item-quality modes retain their behavior.
