...
- Cap four base plus eight category and eight category-product followup calls (twenty total) per snapshot, ten shared snapshots, sixteen tuple positions, 256-byte strings and 128-byte labels. No requests, purchases, refunds, currency guesses or native price/default/stability/completeness claims. Eighteen cumulative actual TOC/slash fixtures (eleven existing, seven new) establish mechanics only; native fixtures and semantics remain unverified. Pinned `CatalogShopDocumentation.lua:259–272` explicitly permits an omitted filter; `673–683,838–846` define observed fields.
