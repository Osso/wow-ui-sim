...
- Cap nine experiment API calls, ten shared snapshots, sixteen tuple positions, 256-byte scalar strings and 128-byte labels. Exclude crafting/recrafting, requests, allocation, setters, reagent-link queries, mutations and native/atlas/error-semantic conclusions. Thirteen local actual TOC/slash fixtures cover recorder mechanics only, not native behavior or final acceptance. Existing schematic/item-quality modes retain their behavior.
