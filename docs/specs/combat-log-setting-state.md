...
Follow-up `3d69ec6ea` writes distinct settings in both environments: first `(true, 180)`, then second `(true, 120)` and `(false, 60)`, asserting neither write leaks across environments. Independent `/tmp/verify-combatlog-settings-isolation-ledger.json` at `3d69ec6ea` records fresh 5/5 each on retail 12.0.0/12.0.5/12.0.7, fmt/readability PASS and exact unchanged-runtime gate reuse. This is stored-setting proof only; it does not alter runtime behavior or the initial 5/5 evidence. Four bounded audit credits do not attach the earlier broad fixture tests as native proof.
