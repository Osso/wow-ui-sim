...
- [x] Forever resolves `[Family]` to `Mainline` and `[Game]` to `Camelot`, accepting `classic`/`camelot` tags without treating `vanilla` as an alias; matching exclusion annotations suppress files and dependencies.
