...
- [ ] Create independent userdata objects supporting `AddBreakpoint`, `ClearBreakpoints`, `Copy`, `GetBreakpoints`, `SetBreakpoints`, and `FormatNumber`.
- [ ] Select the greatest threshold not exceeding the original input; apply breakpoint step rounding, then minimum/maximum clamps.
- [ ] Apply each component's division, remainder and step rounding in that order; format the resulting numeric arguments with the configured format string.
- [ ] Publish `Enum.NumericRuleFormatRounding` and metadata with Nearest=0, Up=1, Down=2. Omitted rounding uses the documented Nearest default.
- [ ] Copy configuration on input, readback and formatter cloning; reject invalid replacement without modifying prior rules.
- [ ] Feed native `FormatNumber` output into duration text bindings and FontStrings, while retaining the existing custom table `.Format` contract.
...
Focused GREEN verification is pending. The native behavior for empty/no-matching rule sets, duplicate thresholds and exact nearest-rounding ties is not documented by these sources. This model explicitly errors for those cases rather than inventing output or choosing an undocumented tie policy. Nonfinite numbers, nonpositive steps, zero divisors/moduli and non-UTF-8 formats are rejected. Readback is threshold-ordered; no native ordering parity claim is made.
