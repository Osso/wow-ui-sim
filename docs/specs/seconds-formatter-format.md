...
Focused abbreviation correction proof at `ab2972d99`: RED reproduced three failures (stale `TwoLetters`, reversed widths, and accepted value `3`). After correction, grouped filters `seconds_formatter_format:: seconds_formatter_configuration::` passed **12 PTR** and **9 retail** tests with `--test integration --offline --no-default-features --features sound,gui,client-<profile>`. Tests cover exact complete profile maps/metadata before and after bootstrap, distinct numeric `0/1/2` output, stored/explicit value `3` rejection, locale/GC behavior, and all six configuration tests. Counts also include one existing garden-format test per profile; its eight partial-addon Lua diagnostics occur in RED/GREEN and are not a clean startup claim. Compiler output had no warnings. Logs: `/tmp/formatter-abbreviation-red-ptr.log`, `/tmp/formatter-abbreviation-ab2972d99-{ptr,retail}.log`; ledger: `/tmp/formatter-abbreviation-proof-ledger.json`. No final check/readability/smoke gates or audit artifact updates were run.
