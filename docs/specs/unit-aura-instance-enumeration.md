...
Public aura instance enumeration is published from retail 12.0.0. Retail 12.1 aura containers enumerate public and private aura instance IDs before fetching aura data. API implementations live in `src/c_api/c_unit_auras.rs`, reusing the existing public aura filter/block model and private-aura store.
...
The two ordinary public-query tests are enabled from `retail-12-0-0`; private-query, source-wrapper and secure-enum tests remain gated at `retail-12-1-0`. Historical tests at `33f7fdb39` reached RED 0/2: the first public query returned a non-array because the entire module and registration call were gated at 12.1.0, despite the query body being ungated. Publication now starts at 12.0.0; the enumeration algorithm and test bodies are unchanged. The first publication build (`1ac925277`) exposed a further 12.1.0 gate on `collect_filtered_unit_auras` (E0432). The next build (`02ae25971`) exposed the adapter's gated `aura_matches_filter_string` dependency (E0425). Both helpers are now gated at 12.0.0 without changing their bodies; `IsAuraFilteredOutByInstanceID` publication remains at 12.1.0. Focused retail 12.0.0 GREEN at `ba31c6b94` passes both ordinary tests (2/2, exit 0; six existing warnings). Proof: `/tmp/aura-instance-ids-historical-green-ledger.json`. No broader or later-profile checks were run for this bounded fix.

Private enumeration and the pre-12.1.5 caster-GUID tombstone remain gated at 12.1.0. The actual `GetAuraCasterGUID` implementation remains gated at 12.1.5; sound-trigger enum registration and private helpers retain their 12.1.0 gates. This publication fix does not establish native sorting parity, private/security behavior, or later-profile regression proof.
