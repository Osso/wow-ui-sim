...
PTR 12.1.5 provides `C_Timer.NewTimedSignalMap` as a state-backed C API object for keyed deadlines. The simulator models its timing and userdata contract; it does not emulate secret-value taint restrictions not otherwise modeled.
...
- `src/lua_api/env_runtime.rs`: timer-pass dispatch and next-deadline integration.
- `src/lua_api/state/sim_state.rs`: map state owned by `SimState`.
