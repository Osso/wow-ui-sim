...
| Finite event registration | `4deca63f1`, `725abfb8b` | Adds finite source-documented Forever acceptance for fifteen startup events. `725abfb8b` adds ten generated-documentation `LiteralName` values, including interface transitions, Diel, Discord, ping, and unit events; invented names remain rejected. This is registration acceptance, not producer-state modeling. |
...
| LFG lair category | `ce7395736`, `e7c5c3039` | Publishes `LE_LFG_CATEGORY_LAIR = 8` for Forever. Isolated `Constants.lua` instrumentation identified this exact missing key at line 482; RED (`0/1`) then warning-free GREEN (`1/1`) proves the constant. |
