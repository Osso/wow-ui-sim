...
- Batch five built `0d8f30dc1` with only the untracked ICE artifact present, recorded its command/status/revision/hash in `/tmp/forever-ui-batch5-build.json`, and then exited `1` with **40 records / 56 occurrences**. Artifacts: `/tmp/forever-ui-batch5-startup.stdout`, `/tmp/forever-ui-batch5-startup.stderr`, and `/tmp/wowforever-ui-proof-ledger.json`. This precedes `1b29df37a`; it remains failing startup evidence, not acceptance.
