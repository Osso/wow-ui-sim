## [2026-09-12] audit | Remove unsupported tooltip money bootstrap shim

`8097a844c` removes the bootstrap `GameTooltip_AddMoneyLine` substitute. It incorrectly treated its third argument as prefix text, emitted plain `GetMoneyString` output, and omitted the loaded helper's color choice. Cached `Blizzard_GameTooltip` owns `(self, rawCopper, useRedLineColor)` after normal loading, using `MoneyFormatter` and highlight/red colors; repository search found no pre-load caller or bare-environment contract. Focused loaded mail/tooltip proof is pending, so `added:GameTooltip_AddMoneyLine` must not retain the removed startup test as evidence. See [[tooltip-money-line]] and [tooltip money-line spec](../specs/tooltip-money-line.md).
