...
`b269650ae` credits exactly bounded `SetRaidTarget` and `TextureBase.SetSpriteSheetCell` behavior, expanding existing `GetRaidTargetIndex` credit to `2244 / 1164 / 2`. `5bc5cdb6f` restores the original one recorded plus one synchronous post-mutation `RAID_TARGET_UPDATE`; corrected tests inspect drained records without artificial callback re-delivery. Focused development proof covers GUID-keyed assignments, row-major cells, and direct original `TargetFrame.lua` consumption; historical XML still fails on unsupported `AuraContainer`, so no XML/full-addon/clean-loader claim follows. Nonnil optional sprite dimensions remain unsupported. World markers and `RemoveRaidTargets` remain evidence-required. Eleven affected hashes were renewed; 538 legacy-drift references remain. Follow-up independent verification is active. See [[unit-raid-target-icons]], [sprite cells](../specs/sprite-sheet-cell.md), and [[patch-12-0-0-api-audit]].
