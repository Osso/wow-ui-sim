## [2026-09-14] audit | Pending bounded CombatText active-unit proof

Tests `8113cc6bc` reached RED 0/3: after a write the generic getter was nil and setter results were nonzero, without proving exact setter arity. Runtime `375c71192` passes focused retail 12.0.0 **3/3** in `/tmp/combat-text-active-unit-green-ledger.json`; independent verifier `18860` is pending. Scope is copied explicit `player`/`vehicle` selection, getter-one nil/string, setter-zero returns, repeated writes and environment isolation. Native initial/token/identity behavior, secret/declassified handling, lifecycle, routing, events and consumer execution remain unproven. No credit, totals, manifests or snapshot changed. [Spec](../specs/combat-text-active-unit.md).
