## [2026-09-08] investigation | Stage controlled unit-frame layering capture

Updated [[betterblizzframes-no-portrait-overlay]] after `e5f4a96ae` and `e811240d0` installed controlled screenshot fixtures to desktop retail main. `/tmp/pi-unit-controls-desktop-install.json` records an exact 5/5 SHA-256 match for `UnitFrameLayerProbe.lua`, `.toc`, `README.md`, `deploy.sh`, and `Controls.lua`. The protocol has five comparisons across created, panel-hide-show, and panel-raise phases; screenshot-success gating, 1.1-second spacing, and cleanup on success/failure/timeout/cancel are covered by 9/9 development checks. Original native capture remains `HIGH`/raised `0` for no-portrait versus `MEDIUM`/raised `18→20` for PlayerSpells/SpellBook; it does not establish global ordering. Independent verification passed (`/tmp/pi-unit-controls-verification.md`); native screenshots, measurements, and cleanup-error inspection remain pending. No simulator ordering fix is claimed.

## [2026-09-08] investigation | Explain BetterBlizzFrames no-portrait SpellBook overlay

Added [[betterblizzframes-no-portrait-overlay]] after the `Uther` overlap report. Saved BetterBlizzFrames no-portrait mode creates a `HIGH`-strata PlayerFrame child; SpellBook is `MEDIUM`. Mask sampling did not change order. The user reports native WoW keeps the overlay below SpellBook; requested read-only [UnitFrameLayerProbe](../addons/UnitFrameLayerProbe/README.md) records raw layering/geometry state without altering observed panels or other addons' settings/SavedVariables. Five local protocol cases have valid retained passes across two runs; independent syntax/formatting, persistence and read-only audits passed (`/tmp/pi-unit-layer-probe-verification.md`). Installation and native capture remain pending. No addon, SavedVariables, or simulator overlap correction was made.
