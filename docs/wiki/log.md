...
Updated [[playerspells-runtime-load]] and [talents panel](../specs/talents-panel.md) after the user explicitly accepted the wider window. `/tmp/pi-gui-escape-wide.json` proves actual retail addons and SavedVariables at a 1906-unit canvas: PlayerSpells remains a center panel and one native GUI Escape closes specialization without opening GameMenu. `/tmp/pi-gui-escape-fit-hook.json` preserves the unfixed 1266-unit overflow and the observed BlizzMove/EnhanceQoLMover callbacks without attributing sole cause. No startup-clean claim is added; color-curve identity remains pending in `/tmp/pi-full-addon-model-final.stdout.log`.
