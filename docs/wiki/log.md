...
Added [[betterblizzframes-no-portrait-overlay]] after the `Uther` overlap report. Saved BetterBlizzFrames no-portrait mode creates a `HIGH`-strata PlayerFrame child; SpellBook is `MEDIUM`. Mask sampling did not change order. No addon, SavedVariables, or simulator change was made.
