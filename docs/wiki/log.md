...
Updated [[betterblizzframes-no-portrait-overlay]], [[rendering-pipeline]], [top-level render groups](../specs/toplevel-render-groups.md), and [mask texture](../mask-texture-system.md) from the authoritative controlled retail capture `/tmp/pi-unit-controls-native.lua`, archived ignored at `docs/local/private/probes/UnitFrameLayerProbe-controls-2026-09-08.lua`. Pixel classification `/tmp/pi-native-control-pixels.json` at physical 1440-pixel display height finds BLUE case 1 (HIGH child remains under LOW top-level owner, behind MEDIUM) and RED cases 2–5 (independent HIGH, DIALOG, plain TOOLTIP, actual GameTooltip above MEDIUM) in created, hide/show, and Raise phases. MEDIUM raised values progress from 0 to `33..37` then `38..42`; LOW parent/red controls stay 0, so ordinals are not global z-index values. `f5d9ff91f` implements the core render grouping and getter boundary; input wiring, targeted GREEN, readability, and GUI acceptance remain pending. No private screenshots are versioned.
