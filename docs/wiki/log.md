## [2026-09-14] audit | Speaker-volume runtime proof pending independent verification

Recorded historical `C_CombatAudioAlert.GetSpeakerVolume()` / `SetSpeakerVolume(number)` test boundary: `4b5ac2a4a` RED 0/3 because setter returned nil rather than boolean success; `cfad799bb` targeted retail 12.0.0 GREEN 3/3, exit 0, retained at `/tmp/audio-speaker-volume-green-ledger.json`. Independent verification by agent 18818 remains pending. No credits, totals, manifests or blocker snapshot changed. Native defaults/ranges/success, CVar/callback/persistence/playback/consumer/security behavior and category-volume APIs remain outside the modeled contract.
