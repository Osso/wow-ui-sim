...
`8097a844c` removes the bootstrap `GameTooltip_AddMoneyLine` substitute: it incorrectly treated argument three as prefix text, used plain `GetMoneyString`, and omitted loaded color selection. `d99d0bc72` adds three `retail-12-0-7` focused cases through loaded Blizzard tooltip/mail code: concrete coin-atlas formatting, single-space zero output, false/default highlight versus true red, and real enclosed-money/COD label ordering. `dd701ae2e` replaces the deleted startup bridge evidence and credits only `added:GameTooltip_AddMoneyLine`. `/tmp/tooltip-money-development-ledger.json` records 3/3 passing; its dependency closure has 128 distinct Lua-error headers, so it does not prove clean whole-addon startup. An aborted first invocation had no captured output and is excluded. Independent verification, native historical-client behavior, locale/rendering variation, and whole-addon compatibility remain open. See [[tooltip-money-line]] and [tooltip money-line spec](../specs/tooltip-money-line.md).
