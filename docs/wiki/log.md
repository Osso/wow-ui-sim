...
Existing-runtime tests `bd76679e0` pass initial retail 12.0.0 3/3 for explicit/repeated message-limit storage, one/zero arity, filter/retention independence and distinct two-environment values. No RED or runtime change. This does not alter prior `SetMessageLimit` event proof and makes no bounds, pruning, filtering, default, native, lifecycle or security claim. Independent profile proof remains pending.
