...
Documented `232a6201f`: manual `recraft-reagent-read <label>`, excluded from `all`, constructs two original guarded equipment `ItemLocation` values and reads opaque GUIDs, then uses two original accessible finite tracked recipe IDs, two schematic slots, and two original reagents per slot. It forwards only eligible original GUID/reagent pairs to `IsRecraftReagentValid`, with guarded optional declared fields and bounded final peer/ancestor reauthorization: 23 recorder API calls at most. Twelve local fixtures, sixteen recraft-limit regressions, and nine eligibility regressions prove recorder mechanics only. This is explicit input acceptance, not actual recraft allocation, target/native validity, warning, mutation, crafting, recrafting, or native execution. Updated [[api-contract-probes]] and the index.
