...
`1e069b03b` gives `After` a dedicated no-argument simulator invoker, retaining callback capture and cancellation suppression while leaving `NewTimer`/`NewTicker` proxy delivery unchanged. `a1c187d2f` credits only `changed:C_Timer.After`, renews 53 existing references across 52 rows, and moves retail 12.0.0 to `2261 / 1147 / 2`. `/tmp/timer-after-green-ledger.json` records four 12.0.0 development cases passing: ordinary function/container calls return no values, defer zero-delay execution, invoke once with zero injected arguments, and do not repeat. NewTimer/NewTicker controls are not native After capture. Later-profile and independent verification remain pending; native callback/container identity, coercion, exact timing, cancellation, lifecycle/GC, and unmodified consumers remain open. See [[timer-after-callback-dispatch]] and [[patch-12-0-0-api-audit]].
