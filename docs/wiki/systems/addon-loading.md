...
TOC parsing strips `#` comments, skips `[AllowLoadTextLocale]` lines for non-enUS, and skips `[AllowLoadGameType]` lines unless any inline gametype matches the active profile's allow-list (retail: mainline/standard; wrath: wrath/wrath_classic/classic; mists: mists/mists_classic/classic; era: vanilla/classic_era/classic; anniversary: vanilla/classic_anniversary/classic; Forever: camelot/classic). Inline game types may be comma- or whitespace-delimited, so `[AllowLoadGameType vanilla tbc mainline]` admits the current retail `mainline` file. It substitutes `[Family]` for the profile's family-subdir (Mainline for retail/PTR/Forever; Classic for wrath/mists/era/anniversary), substitutes `[Game]` for the active profile's game subdir (Forever: Camelot), and normalizes backslashes. Forever's Mainline family directory does not widen its annotation allow-list to `mainline`. Path resolution is case-insensitive for Windows/macOS compatibility. See [[client-profiles]] for the full profile-aware tables.

`[Bootstrap]` never reorders files within an eager TOC: `Before.lua → Bootstrap.lua [Bootstrap] → Normal.lua` stays literal. Under the cumulative `retail-12-1-0` startup path (retail 12.1 and PTR), eligible Blizzard LoadOnDemand TOCs with annotated files enter the same dependency-ordered startup stream as `BootstrapOnly` nodes; this is not a global pre-pass. They execute only their annotated entries and return to not-loaded state. The first later full load skips already-executed bootstrap files; later full loads remain idempotent. Classic profiles retain their existing startup selection. Bootstrap files referenced by an active profile's manifest or TOC must be represented in that profile's required-entry set so stale completed caches are rejected when the file is missing. Retail and PTR both require `Blizzard_CooldownBroadcaster_Bootstrap.lua` for the current 12.1 manifest; stale PTR-only filtering previously let retail accept an incomplete cache.

### Retail Encounter Journal publisher

Retail's `Blizzard_EncounterJournal_Mainline.toc` marks the addon LoD and its bootstrap entry `[Bootstrap]`. That file defines `CanShowEncounterJournal`, consumed by `EJMicroButtonMixin:OnClick` at `MainMenuBarMicroButtons.lua:1635`. Restricting bootstrap startup to `retail-12-1-5` left this retail 12.1 global nil while `KeybindFrames_InQuickKeybindMode` already existed. The actual click reproduced the nil call and left the journal closed (`/tmp/pi-retail-micro-menu-red.*`). Promoting the existing gate to `retail-12-1-0` loads the publisher rather than shimming the query. The process-level [micro-menu test](../../../tests/micro_menu.rs) now uses normal startup and requires an error-free open/close sequence; GREEN is pending a build slot.

## Load Flow (`src/loader/addon.rs`)

During startup, discovery loads non-LoadOnDemand roots plus explicit LoadOnDemand startup roots and their hard dependencies. The cumulative `retail-12-1-0` epoch additionally places eligible Blizzard LoD TOCs with `[Bootstrap]` entries into that ordered stream as bootstrap-only nodes. Full startup roots run complete TOCs; bootstrap-only nodes run only annotations and do not receive full-load `ADDON_LOADED` delivery. The first later `C_AddOns.LoadAddOn` runs remaining files; later calls are idempotent. Other LoD addons remain metadata-only until explicitly loaded.
...
Third-party addons load after the Blizzard startup pass. Under `retail-12-1-0`, eligible enabled third-party LoadOnDemand addons execute only `[Bootstrap]` entries at their ordinary startup position. Classic selection is unchanged. Non-LoadOnDemand third-party addons execute `[Bootstrap]` inline with normal files.
