...
`19416ff84` makes `Copy` and `Assign` transfer only modeled duration timing and optional clock binding. Development GREEN covers seven `duration_copy_` cases: copied start/duration/rate, independent later timing mutation, one Copy return and no Assign returns, self-assignment, receiver identity/custom fields, shared clock-object advancement with independently mutable bindings, clearing a receiver clock from an unbound source, and rejection of invalid sources before mutation. Copying a clock reference, retaining receiver identity/custom fields, and all binding policies are simulator choices; no native copy/assignment, identity, coercion, security, lifecycle/GC, or consumer behavior is inferred. Independent verification remains pending.
