...
The current behavior-based eligibility audit records exactly 1,965 tests in the dedicated default-retail prefork target: 1,956 marker-generated full-environment cases and 9 manual/nested prefork cases. Fourteen settled behaviors moved one-for-one from ordinary integration: nine `party_frame_tree` cases with child-local party-state mutation and five `objective_tracker_tree` cases with child-local frame-state mutation. Integration now lists 8,412 tests. The ordinary startup-like scan has 290 remaining tests and zero eligible cases for the finalized shared parent. Final audit artifacts are `/tmp/prefork-final-eligibility.json` and `/tmp/prefork-final-registry-list.txt`; see [Final eligibility classification](#final-eligibility-classification) for exclusions.
