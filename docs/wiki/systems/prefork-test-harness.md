...
The last verified behavior-based eligibility audit recorded 1,965 tests in the dedicated default-retail prefork target: 1,956 marker-generated full-environment cases and 9 manual/nested prefork cases. Fourteen settled behaviors had moved one-for-one from ordinary integration: nine `party_frame_tree` cases with child-local party-state mutation and five `objective_tracker_tree` cases with child-local frame-state mutation. Commit `e7c1b8576` then marked 17 additional normal complete-retail-startup cases for the same parent: two AddOnList frame-surface cases, three AddOnList global-surface cases, one AddOnList mixin-surface case, five micro-menu/game-menu behaviors, and six `ShowUIPanel`/`HideUIPanel` CharacterFrame behaviors. Post-commit inventory and runtime measurements remain unverified. Final audit artifacts are `/tmp/prefork-final-eligibility.json` and `/tmp/prefork-final-registry-list.txt`; see [Final eligibility classification](#final-eligibility-classification) for exclusions.
