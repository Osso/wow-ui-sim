...
**Settings panel surface** — The post-load temporary workaround in `settings_surface_defaults.rs` reconciles the canonical `_G.SettingsPanel`/`Settings` pair when Blizzard replaces either surface. It adds only absent category IDs and `Settings.OpenToCategory`, preserving existing members; reapplying it is idempotent for the same panel identity. Retail 12.1 also pins the 13 housing Settings labels/tooltips required for the canonical Interface registrant to finish. `C_SettingsUtil.OpenSettingsPanel(categoryID, scrollTarget)` is modeled: it forwards to `SettingsPanelMixin.OpenToCategory(SettingsPanel, categoryID, scrollTarget)`, defaults an absent category ID to `0`, and is a no-op if either required surface is absent. `C_SettingsUtil.NotifySettingsLoaded` remains unsupported; category/element scrolling semantics beyond the supplied target remain unmodeled.
...
**Base spell aura secrecy** — `C_Secrets.GetSpellAuraSecrecy` uses generated native aura attributes and the existing spell-identifier resolver, not a combat-policy guess. Dual-flag data is retained and rejected explicitly when queried. Four focused cases passed; conditional aura secrecy enforcement remains unmodeled. See [contract, provenance, and focused proof](../../specs/spell-aura-secrecy.md).

**Duration bindings and forbidden aspects** — Duration text bindings are validated userdata handles: `Assign` and `Copy` copy configuration while retaining documented object handles, and `securecopy(options)` preserves a registered binding handle without treating copied tables as bindings. XML forbidden-aspect names resolve through active enums; creation-time hierarchy/layout propagation applies to literal/runtime templates and new children. Focused duration (6) and forbidden-aspect (4) cases passed; dynamic propagation over existing graphs remains unmodeled. See [duration binding](../../specs/duration-text-binding.md) and [forbidden aspects](../../specs/forbidden-aspect-inheritance.md).

C_Timer (After, NewTimer, NewTicker), C_Map (stub), C_Item (`IsConsumableItem`, `IsEquippableItem`, and `IsItemInRange` are state-backed; other listed item methods remain mixed implemented/stubbed), C_Container (`GetBagSlotFlag`, `SetBagSlotFlag`, `GetBackpackAutosortDisabled`, and `GetBackpackSellJunkDisabled` share state-backed bag-slot flags), C_Spell (`GetSpellQueueWindow()` reads the current `SpellQueueWindow` CVar and returns its numeric value when parseable), C_System (GetLocale → "enUS"), C_EditMode (GetLayouts), C_CatalogShop (`GetVCProductInfos()` returns a fresh empty table), C_ChromieTime (retail/PTR empty-state queries and no-op actions), C_SettingsUtil (`OpenSettingsPanel` forwards to the current `SettingsPanelMixin.OpenToCategory` surface), C_StringUtil (decimal escaping plus retail-family format/pattern escaping and conditional wrapping), C_Quest, C_AchievementInfo, C_ClassTalents, C_Guild, C_LFGList, C_Mail, C_ActionBar — most return nil/false/0 stubs.

**C_StringUtil boundary** — `EscapeDecimalNonPrintables` preserves valid UTF-8 and replaces ASCII control bytes except tab/newline/carriage return, plus invalid UTF-8 bytes, with decimal escapes. On retail and PTR, `EscapeLuaFormatString(text)` doubles `%`; `EscapeLuaPatterns(text)` prefixes each Lua pattern character (`^`, `$`, `(`, `)`, `%`, `.`, `[`, `]`, `*`, `+`, `-`, `?`) with `%`; and `WrapString(infix, prefix, suffix)` treats missing optional affixes as empty strings and returns `""` for an empty infix. All three preserve other Lua string bytes, including NUL and invalid UTF-8. Local `StringUtilDocumentation.lua` establishes signatures and transformations only, not live-client error, taint, or secret-value behavior.
