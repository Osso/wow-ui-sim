...
**C_StringUtil boundary** — `EscapeDecimalNonPrintables` preserves valid UTF-8 and replaces ASCII control bytes except tab/newline/carriage return, plus invalid UTF-8 bytes, with decimal escapes. Retail-family `EscapeLuaFormatString(text)` doubles `%`; `EscapeLuaPatterns(text)` prefixes each Lua pattern character (`^`, `$`, `(`, `)`, `%`, `.`, `[`, `]`, `*`, `+`, `-`, `?`) with `%`; and `WrapString(infix, prefix, suffix)` treats missing optional affixes as empty strings and returns `""` for an empty infix. Local `StringUtilDocumentation.lua` establishes signatures and transformations only, not live-client error, taint, or secret-value behavior.
