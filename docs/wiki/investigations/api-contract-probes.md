...
Runtime `ffd3b08ca` adds manual `/apicontract color-curves <label>`, also excluded from `all`. It constructs four owned color curves, records raw state, point lookups and packed/unpacked evaluation, and observes reset/copy mutation without classifying defaults or isolation. Returned tables expose only guarded raw RGBA fields; userdata stays opaque and no returned methods or metamethods run. Four new fixtures extend the prior 41-fixture harness; they are not 45 new native cases. Native userdata field representation, identity, coercion, security, removal/replacement and alternate interpolation behavior remain unexecuted.
