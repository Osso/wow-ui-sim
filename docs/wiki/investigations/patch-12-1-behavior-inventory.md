...
| `Patch12_1.ForbiddenAspects.ScriptedInput` | best-effort  | behavioral  | Forbidden Aspects | changed | A uniform simulator policy rejects only annotated Lua `Click`, `SetFocus`, `ClearFocus`, and `SetCursorPosition` calls before state/callback changes; physical input and text methods remain available. Native security is unproven. |
| `Patch12_1.ForbiddenAspects.QueryFocus` | best-effort  | behavioral  | Forbidden Aspects | changed | A uniform simulator policy rejects only annotated `HasFocus` and `IsMouseMotionFocus` calls; global focus queries and `IsMouseOver` remain available. Native security is unproven. |
