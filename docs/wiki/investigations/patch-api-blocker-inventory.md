...
## Unit-filtered global callback residuals

Two bounded credits cover ordinary `RegisterUnitEventCallback`/`UnregisterUnitEventCallback`: actual pinned `Event.lua` wrappers and `FunctionContainer` userdata, exact first-payload unit filtering, nil owner/payload, both dispatch paths, event/container/unit identity removal, zero-return arity and environment isolation. Runtime `0a0ac416f`, publication `0835b3c94` and tests `c1d401ba3` progress RED 0/4 to independent 12/12 each retail 12.0.0/12.0.5/12.0.7, including four unit and eight global regressions; fallback proof is 2/2 retail and 3/3 Mists. Fmt/check/default builds/startup `[]` pass. Metadata `539c44c57` proves two credits, 63 renewals, 20 additions, **15,124 fresh / zero stale**, six bindings and validator exit 0 / 3,410 rows; totals **2351 / 1057 / 2**, snapshot **1,083 / 282**. Exact matching/order/duplicates/snapshot/global-before-unit order/validation are simulator policies; aliases/lists, native ordering/errors/eligibility, secrecy/taint, full-LoD, mutation/recursion/error recovery and frame ordering remain unverified. One registration-length signal and eight Mists failures remain. [Spec](../../specs/unit-event-callbacks.md); [[patch-12-0-0-api-audit]].
