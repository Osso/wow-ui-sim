...
## Unit-filtered global callback lifecycle pending proof

Runtime `0a0ac416f`, publication `0835b3c94` and tests `c1d401ba3` model ordinary `RegisterUnitEventCallback`/`UnregisterUnitEventCallback`; RED is 0/4 because initial callbacks were not delivered. The registry is separate from frame listeners and matches one nonempty registered unit string exactly against the first event payload; functions and modeled `FunctionContainer` userdata receive `nil, ...payload`. It snapshots global callbacks before matching unit callbacks. Integrated historical-profile/startup proof, provenance and credits are pending. Exact matching/order/duplicates/snapshot/global-before-unit order/validation are simulator policies; aliases/lists, native ordering/errors/eligibility, secrecy/taint, full-LoD, mutation/recursion/error recovery and frame ordering remain unverified. [Spec](../../specs/unit-event-callbacks.md); [[patch-12-0-0-api-audit]].
