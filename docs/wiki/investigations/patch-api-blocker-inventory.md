...
**777 rows now link to 205 plans** in the JSON: 619 rows use 47 family-specific plans, and 158 rows preserve explicit probe requirements already written in their manifest notes. The other **344 rows retain routing protocols only**; extraction of a source requirement does not count as fresh semantic validation. All 205 plans remain `ready_to_execute: false`: native fixtures, an exact producer/consumer or an unresolved contract is missing. Plans specify observations, not guessed native outcomes.
