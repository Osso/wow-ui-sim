...
The trusted host observer recorded `PAINTED_GREEN` before removal, then a stale assignment after `A_Admin.RemoveBuff(19750)`. **Diagnosis correction:** `c6d970cf3` corrects only the `FireEvent`/`A_Admin.FireEvent` path, whose legacy dispatcher selected the normal binding. `A_Admin.AddBuff`/`RemoveBuff` instead call `fire_named_event_state`, which already dispatches intrinsic bindings; the passing native cold-event lifecycle test is therefore not proof that `c6d970cf3` fixed the GUI removal. `/tmp/ellesmere-forever/aura-post-removal-state-ledger.json` identifies the current route: after `ReloadFrames`, visible `pball` frame 10 has `UNIT_AURA` unregistered and `dynamicEvents` nil because recursive visibility dispatch calls only normal `OnShow`, skipping AuraContainer's intrinsic re-registration. That visibility diagnosis remains under implementation/proof. The aura group existed; this is neither a loader-inheritance nor absent-group diagnosis.
...
`fc83c50cc` implements the next demonstrated native handoffs without generic coercion: authenticated wrapped inputs at `SetPoint`, size setters, and `SetTexture`. Anchor origin is recorded per point; width/height origins are independent; texture source origin is private. Tainted geometry/source readouts conservatively reject secret-origin state through the demonstrated direct, parent, and relative-anchor paths. This readout policy is an **informed simulator guess**: native sources prove wrapped inputs, while Forever-client probe evidence is unavailable. The same commit adds focused endpoint tests for authenticated `SetPoint`, `SetSize`, and `Texture:SetTexture`, but no focused GREEN or GUI replay is claimed yet.
...
- Compile and run focused GREEN for the uncompiled TOC/profile, TargetFrame diagnostic, unit-relationship, secret-duration-core, widget, duration-text-binding, and geometry/texture endpoint slices. Secret-origin flags guard only the direct modeled reads named above; they do not establish general secrecy enforcement.
