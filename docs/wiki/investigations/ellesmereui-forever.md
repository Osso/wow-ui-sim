...
- Run compiled GREEN for `9c56b683d` duration-binding availability, including formatter-to-FontString behavior, before attributing later AuraContainer failures.
- Establish registration preservation across `Blizzard_EnvironmentCleanup` before changing SecondsFormatter exposure.
- Audit narrow Forever gates for `CustomAuraButtonUpdateMode`, `C_AuraContainerUtil`, and `C_Secrets`; do not enable a whole Retail epoch.
